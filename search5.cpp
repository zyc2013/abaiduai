...
int ReadJson_much_string(const std::string & strValue, std::string obj, std::string key, std::string tag,  std::string &value)
{
	Json::Reader reader;  
	Json::Value  root; 
	int ret = -1;
	...
	if (reader.parse(strValue, root))
	{
		...
		Json::Value arrayObj1 = arrayObj[key];
		
	value = arrayObj1[0][tag].asString();//数组元素需要两个角标表示	
		//value  = arrayObj1[tag].asString();
//cout<<arrayObj1.toStyledString()<<endl;
		ret = 0;
...
cout<<"search_result="<<search_result<<endl;
	std::string log_id;
	ret = ReadJson_string(search_result, "error_msg", log_id);
	if(0==ret)
	{
		cout << " error_msg = " << log_id <<endl;
	}
	std::string user_id;
 	ret =ReadJson_much_string(search_result, "result","user_list","user_id", user_id);
	if(0==ret)
	{
